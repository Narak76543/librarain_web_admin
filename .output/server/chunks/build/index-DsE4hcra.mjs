...
import { u as useApi } from './useApi-r5bmIrpy.mjs';
import '../_/nitro.mjs';
import 'node:http';
import 'node:https';
import 'node:events';
  ...
  setup(__props) {
      ...
      if (res && res.ok) {
        books.value = res.data.books.map((b) => {
          ...
          return {
            ...
            category: ((_a = b.category) == null ? void 0 : _a.name) || "Uncategorized",
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(ssrRenderComponent(_component_DataTable, {
        ...
        featured: withCtx(({ row }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<label class="relative inline-flex items-center cursor-pointer"${_scopeId}><input type="checkbox" class="sr-only peer"${ssrIncludeBooleanAttr(row.featured) ? " checked" : ""}${_scopeId}><div class="w-9 h-5 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[&#39;&#39;] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-info"${_scopeId}></div></label>`);
          } else {
            return [
              createVNode("label", { class: "relative inline-flex items-center cursor-pointer" }, [
                createVNode("input", {
                  ...
                  onChange: ($event) => toggleFeatured(row)
                }, null, 40, ["checked", "onChange"]),
...
//# sourceMappingURL=index-DsE4hcra.mjs.map
