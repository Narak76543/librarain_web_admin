...
import { u as useApi } from "./useApi-r5bmIrpy.js";
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const isLockedUser = (user) => {
      ...
      const lockedUntilVal = user.locked_until || user.lockedUntil || user.account_status?.locked_until || user.account_status?.lockedUntil || user.raw?.account_status?.lockedUntil;
      if (lockedUntilVal) {
        const lockDate = new Date(lockedUntilVal);
        if (lockDate > /* @__PURE__ */ new Date()) return true;
      }
      const failedAttempts = user.failed_login_attempts ?? user.failedLoginAttempts ?? user.account_status?.failed_login_attempts ?? user.account_status?.failedLoginAttempts ?? user.raw?.failed_login_attempts ?? 0;
...
//# sourceMappingURL=index-dWnOvJuL.js.map
